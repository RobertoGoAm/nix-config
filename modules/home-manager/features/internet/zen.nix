...
#   zen.spaces.work   = { name = "..."; routes = [ "host" "host" ]; };
#   zen.spaces.client = { name = "..."; routes = [ "host" ]; };
  ...
  programs.zen-browser = {
    ...
    profiles.${user} = {
      ...
      extensions = {
        ...
        # The rule below is the shape the extension takes: the work jars, asleep outside
        # working hours. Containers 2 and 3 by id rather than by name -- the ids are the
        # ones assigned above, they survive a rename, and the names live outside this
        # repo. ~windows~ makes the rule awake only in the hours it lists, so nothing is
        # discarded during the working day; within a window a tab still has to have gone
        # ~idleMinutes~ untouched.

        # Three windows rather than the one "18:00 to 09:00" it reads as. A window whose
        # end is at or before its start crosses midnight and belongs to the day it opens
        # on, which the extension handles -- but a Monday morning has no workday evening
        # before it, so a single crossing window leaves 00:00 to 09:00 on Monday awake
        # and everything else asleep. Split at midnight there is no such corner: evening,
        # early morning, and the weekend entire.

        # An always-on rule is one more attrset -- containers, an idle time, no
        # ~windows~ -- if the daytime pile-up ever wants the same treatment.

        # The settings reach the extension as ~storage.local~, which is what
        # home-manager writes. The extension prefers ~storage.managed~ where a policy
        # exists, but this module only passes ~policies~ into the Linux package, so on
        # darwin the local copy is the one that arrives.
