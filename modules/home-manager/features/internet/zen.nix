...
let
  ...
  # Outside 09:00-18:00, Monday to Friday: the hours in which a rule carrying
  # this is allowed to act, and by omission the hours it must not.
  #
  # Three windows rather than the one "18:00 to 09:00" it reads as. A window
  # ending at or before it starts crosses midnight and belongs to the day it
  # opens on, which the extension handles -- but Monday morning has no workday
  # evening before it, so a single crossing window leaves 00:00 to 09:00 on
  # Monday awake and everything else asleep. Split at midnight there is no such
  # corner.
  ...
  programs.zen-browser = {
    ...
    profiles.${user} = {
      ...
      extensions = {
        ...
        # it has had its time met, so the shortest applicable threshold is the one that
        # acts and the order they appear in does not matter. The client's mail is the
        # reason that matters: it is the one thing that must survive a working day
        # untouched, so it is not in the general list at all but carries its own rule,
        # awake only outside office hours and impatient once it is -- five minutes, not
        # thirty, because after six in the evening an inbox left open is just resident
        # memory.
        ...
        # getting it back costs a reload. Give those rules ~windows = offHours~ too if
        # the working day should be left entirely alone.
