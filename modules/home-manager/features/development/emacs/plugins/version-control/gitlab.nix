...
}:
{
  programs.emacs.extraConfig = lib.mkOrder 1460 ''
    ...
    ;;
    ;; Outside `with-eval-after-load', unlike everything above. Those act on the
    ;; topic under point, so forge is loaded by the time any of them can be
    ;; reached; this one is the way in, pressed from wherever you happen to be,
    ;; and a command that does not exist until forge loads is not a command --
    ;; Emacs says so in as many words: "Wrong type argument: commandp".
    ;;
    ;; forge's own reader asks `forge-get-repository' for a *tracked* repository
    ;; and errors when there is none, which is exactly where a global entry
    ;; point is invoked from. So it is used only where it works: inside a
    ;; tracked repository you get completion over the members, anywhere else a
    ;; plain prompt. The answer is kept for the session, and a prefix argument
    ;; asks again. It is never written down -- an account name, in a public
    ;; repository.
