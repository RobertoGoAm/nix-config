...
}:
{
  ...
  programs.emacs.extraConfig = lib.mkOrder 1400 ''
    ...
    (my/leader
      ...
      ;; notes — the Obsidian vault. SPC n o is fd over the vault and answers on
      ;; a cold session; SPC n j is obsidian.el's own jump, which knows titles
      ;; and aliases and pays for the index the first time it is used.
      "n"  '(:ignore t :which-key "notes")
      "nn" '(my/obsidian-new :which-key "new note")
      "nN" '(my/obsidian-new-from-template :which-key "new note from template")
      "no" '(my/obsidian-open-note :which-key "open note")
      "nj" '(my/obsidian-jump :which-key "jump by title or alias")
      "ns" '(my/obsidian-search :which-key "search vault (C-u: backups too)")
      "nd" '(my/obsidian-daily-note :which-key "today's daily note")
      "nh" '(my/obsidian-headings :which-key "heading in this note")
      "nt" '(my/obsidian-find-tag :which-key "notes with tag")
      "nT" '(my/obsidian-insert-tag :which-key "insert tag")
      "nl" '(my/obsidian-insert-wikilink :which-key "insert wiki link")
      "nL" '(my/obsidian-insert-link :which-key "insert markdown link")
      "nx" '(my/obsidian-remove-link :which-key "unlink")
      "nf" '(my/obsidian-follow-link :which-key "follow link at point")
      "nF" '(my/obsidian-jump-back :which-key "back from link")
      "nb" '(my/obsidian-backlinks :which-key "backlinks")
      "nB" '(my/obsidian-toggle-backlinks :which-key "backlinks panel")
      "nm" '(my/obsidian-move-note :which-key "move to folder")
      "nr" '(my/markdown-toggle-render :which-key "rendered / raw markdown")
      "np" '(markdown-live-preview-mode :which-key "rendered HTML preview")
      "nu" '(my/obsidian-rescan :which-key "rebuild vault index")
      "nO" '(my/obsidian-open-app :which-key "open in Obsidian app")
      ...
      "$b" '(my/hledger-budget :which-key "envelopes (C-u: pick month)")
      ...
      "$R" '(my/hledger-renta :which-key "estimate the AEAT settlement")
      "$v" '(my/hledger-charts :which-key "charts: net worth, in vs out")
      "$w" '(my/hledger-sweep :which-key "sweep the leftover at the mortgage")
      "$u" '(my/hledger-catch-up :which-key "catch the funds up to this month")
      "$p" '(my/hledger-pace :which-key "pace the dated goals to their dates")
      ...
      ;; test -- jest.el's own commands, and its popup for everything else.
      ;;
      ;; f is the plain "run this file" and F is the dwim, which is the opposite
      ;; way round from jest.el's own popup. The dwim asks projectile whether
      ;; the current file is a test, and projectile calls a JS test `.test'
      ;; whatever the repository actually names them -- so on a `.spec' project
      ;; it is wrong until that project's .dir-locals.el says otherwise. See the
      ;; note in plugins/code/test.nix. The one that is always right gets the
      ;; lowercase key.
      ;;
      ;; No key for the output buffer: every run displays it. None for watch or
      ;; coverage either -- those are switches on the popup, not commands.
      "t"  '(:ignore t :which-key "test")
      "tt" '(jest-popup :which-key "menu (watch, coverage, -t, --config)")
      "ta" '(jest :which-key "all")
      "tf" '(jest-file :which-key "this file")
      "tF" '(jest-file-dwim :which-key "this file's test (needs projectile suffix)")
      "tn" '(jest-function :which-key "test at point")
      "tr" '(jest-repeat :which-key "repeat last run")
