...
in
{
  programs.emacs.extraConfig = lib.mkOrder 1450 ''
    ...
    ;; Declared here because the `let' below binds it, and this file is compiled
    ;; without claude-code-ide loaded: an unbound-at-compile-time symbol is bound
    ;; lexically, and the flag never reaches the CLI.
