...
in
{
  ...
  programs.emacs.extraConfig = ''
    ...
    (defun my/claude-sessions ()
      "Show every Claude session, in every project, with its state.
    A row per live session, glyph first: `?' waiting on you, `✓' finished,
    `✗' failed, `⚙︎' working, `🔔' gone quiet. RET switches into one, SPC
    switches but keeps the point here, s starts a session on a project row
    that has none, and g refreshes."
      (interactive)
      (claude-code-ide-manager-toggle-global-sidebar))

    (defun my/claude-set-session-state (buffer-name state)
