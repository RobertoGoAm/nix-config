...
}:
{
  ...
  programs.emacs.extraConfig = ''
    ...
    ;; compose.yml has no mode of its own here: it is ordinary YAML, and the
    ;; key completion and validation a dedicated mode would offer already
    ;; arrive through lsp-yaml's schemastore catalogue (code/languages.nix).
