...
in
{
  ...
  programs.emacs.extraConfig = ''
    ...
    interest is an envelope; the rest converts cash into a smaller debt. That
    is as real a claim on the money as any envelope, and nothing was counting
    it -- so the sweep would send it at the loan and the account would then be
    short by the principal when the direct debit landed, every month on the
    eighth."
      (apply #'+
             (mapcar (lambda (row)
      ...
      (let* ((period (my/hledger-budget--month-string my/hledger-budget--time))
        ...
        (let* ((earmarked
          ...
          (let* ((promised (my/hledger--promised))
                 ...
                 ;; Asked again rather than reused: the saving block's binding
                 ;; is inside its own `let*' and does not reach here.
            ...
            (insert (format "  %-26s %9.2f   what the goals still need\n"
                            "reserved" reserved))
            (insert (format "  %-26s %9.2f   the cuota's principal, still to leave\n"
                            "the loan" principal))
    ...
    (defun my/hledger-balances ()
      "Every real balance at once: the bank accounts, the cash, the card.

    `assets' and `liabilities' rather than the balance sheet behind SPC $ n,
    because the question this answers is what each account holds right now,
    not how the year moved.

    In tree form, so the envelopes appear underneath the account they are
    carved out of. That matters here: the bank's own figure is the total of
    the tree, not the parent line -- which is why the reconciliation
    assertions in the journal are written `==*' rather than `='."
      (interactive)
      (my/hledger--report "*hledger balances*"
                          "balance" "assets" "liabilities" "--tree"))
