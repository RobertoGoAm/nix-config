  ...
  programs.emacs.extraConfig = ''
    ...
    the badge rather than showing an error in the modeline."
        ...
        (defun my/status-telega ()
          "Unmuted Telegram count, or nil when there is nothing unread.

        Unmuted rather than total: muted groups are muted precisely so they do not
        ask for attention. bound-and-true-p throughout because telega is
        autoloaded and in most sessions these never come into existence."
        ...
        (defun my/status-string ()
          "The ambient status: track, unread mail, unread Telegram.
        Empty when there is nothing to say, so the bar stays quiet."
          (string-join
           (delq nil
                 (list (unless (string-empty-p my/status-music)
                         (propertize my/status-music 'face 'shadow))
                       (unless (string-empty-p my/status-mail)
                         (propertize my/status-mail 'face 'success))
                       (my/status-telega)))
           "   "))
