  ...
  programs.emacs.extraConfig = lib.mkOrder 300 ''
        ...
        (defconst my/tokyonight
          '((bg       . "#24283b")
            ...
            (red      . "#f7768e")
            (yellow   . "#e0af68")
            ;; The diff group from the same palette, for the two-pane ediff
            ;; compare in version-control/git.nix. diff-add and diff-del are
            ;; tokyonight's own diff.add and diff.delete; the -hl pair is each of
            ;; them mixed halfway to the matching git.add / git.delete, because
            ;; ediff paints the changed words inside an already-tinted hunk and
            ;; the two tints have to be told apart.
            (diff-add    . "#20303b")
            (diff-del    . "#37222c")
            (diff-add-hl . "#326673")
            (diff-del-hl . "#643740"))
