...

{
  programs.zsh = {
    ...
    initContent = ''
      ...
      #
      # Set NIX_REBUILD_HERE=1 to stay in the current terminal. Over SSH that
      # happens automatically: `open -a Terminal` succeeds on a machine with a
      # console session, but the window opens on *that machine's* screen, so
      # from an SSH session the rebuild looks like it silently did nothing while
      # a window sits waiting for a password somewhere you cannot see. The App
      # Management grant is worth the indirection when there is a screen in
      # front of you and worthless when there is not.
      ...
      # escalated.
      nix-build() {
        if [[ "$TERM_PROGRAM" != "Apple_Terminal" && -z "$NIX_REBUILD_HERE" && -z "$SSH_CONNECTION" ]]; then
