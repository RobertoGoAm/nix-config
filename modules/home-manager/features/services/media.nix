...
in
{
  ...
  # ffmpeg and the web UI are both handled already, and passing either again is
  # not a no-op. The nixpkgs wrapper ends in

  # :   exec .../jellyfin --ffmpeg=.../ffmpeg --webdir=.../jellyfin-web "$@"

  # so a =--webdir= of our own arrives second and Jellyfin refuses to start at
  # all: "Option 'w, webdir' is defined multiple times", followed by its help,
  # into the error log. Nothing listens on the port and the agent flaps.

  # Which is also why there is no jellyfin-ffmpeg on PATH here. The wrapper names
  # the binary by absolute path, so PATH never comes into it.
